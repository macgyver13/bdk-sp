...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        Commands::ScanCbf {
            ...
            let sync_height = if let (Some(height), Some(hash)) = (height, hash) {
                BlockId { height, hash }
                ...
                let checkpoint = wallet.chain().get(wallet.birthday.height).expect("should be something");
                let height = checkpoint.height();
                let hash = checkpoint.hash();
                BlockId { height, hash }
            };

            tracing::info!("Synchronizing from block {}:{}", sync_height.hash, sync_height.height);

            // Set up the light client
            let checkpoint =
                HeaderCheckpoint::closest_checkpoint_below_height(sync_height.height, wallet.network());
            ...
            let mut filter_subscriber =
                FilterSubscriber::new(db_buffer, event_rx, changes_tx, sync_height.height);
